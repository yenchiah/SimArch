...
function [ display_new map_path goal_row goal_col ] = searchTarget( pos_row,pos_col,obstacle,display,scale )
    % the weight factor of weighted A* search
    weight = 10;
    heuristic_noise_variance = 1000;
    
    % randomly find display as a goal
    [rows cols] = size(display);
    ...
    goal_col = g_col;
    display_new = display;
    target = display(g_row,g_col);
    display_new(display==target) = 255;
